...
auth.post('/register', async (c) => {
  ...
  const token = await sign({ sub: result[0].id, email, exp: Math.floor(Date.now() / 1000) + 86400 }, c.env.JWT_SECRET)
...
auth.post('/login', async (c) => {
  ...
  const token = await sign({ sub: user.id, email, exp: Math.floor(Date.now() / 1000) + 86400 }, c.env.JWT_SECRET)
